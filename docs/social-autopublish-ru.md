...
- Пишите **один** текст: эмодзи Unicode (🔧 ✅), ссылки `https://` или `isnix.ru/...`.
- Подвал **не** вставляйте в `posts/*.txt` — он подставится автоматически.

### Разметка в `posts/*.txt`

| Синтаксис | Discord | Telegram | ВК |
|-----------|---------|----------|-----|
| `**жирный**` | embed | **жирный** | жирный снят |
| `*курсив*` | — | *курсив* | снят |
| `` `команда` `` | — | моноширинный | снят |
| `__подчёркивание__` | — | подчёркнутый | снят |
| `~~зачёркнутый~~` | — | зачёркнутый | снят |
| `## Заголовок` | title | жирная строка | без `##` |

Telegram публикуется с `parse_mode=HTML` (автоконвертация из markdown-подобного синтаксиса).
