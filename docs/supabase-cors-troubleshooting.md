...
## 8. Нет VPN, AdBlock выключен — всё равно ERR_CONNECTION_RESET

Часто режет **провайдер или ПК** (не расширение браузера):

1. **Точка доступа с телефона** — раздай Wi‑Fi с телефона, открой `https://isnix.ru/account.html` с компьютера. Если заработало — блокирует домашний Wi‑Fi/провайдер.
2. **DNS в Windows** — Параметры → Сеть → DNS: `1.1.1.1` и `1.0.0.1` (или `8.8.8.8`).
3. **Безопасный DNS в браузере** — Chrome/Edge → Настройки → Безопасность → **выключи** «Использовать безопасный DNS».
4. **Антивирус** — Kaspersky, Dr.Web, ESET часто обрывают HTTPS к `*.supabase.co`. Временно отключи «проверку защищённых соединений» или добавь исключение для `supabase.co`.
5. **Другой браузер** — Firefox или Edge, если сейчас Яндекс.Браузер / Opera.
6. **Cloudflare 1.1.1.1 (WARP)** — бесплатное приложение, не классический VPN; у многих снимает блокировку без платного VPN.

**Проверка в новой вкладке** (должен быть текст JSON про `apikey`, а не пустая страница):

```
https://yfrlgeztbaebdapdnefy.supabase.co/auth/v1/health
```

- Видишь JSON → сеть до Supabase **есть**, жми на сайте «Проверить связь» и «Повторить загрузку».
- «Сброс соединения» / пусто → блокировка на уровне сети или антивируса на **этом** устройстве.

В личном кабинете: кнопка **«Проверить связь»** под жёлтым предупреждением.
