...
- **1.0.9** — теги в TAB после рестарта: чтение из `isnix-opac-tab.json` для оффлайн игроков, защита от затирания файла при ошибке загрузки, повторная инициализация OPAC API, миграция `tag_lead_reset: &r`.
